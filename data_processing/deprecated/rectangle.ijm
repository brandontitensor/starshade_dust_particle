                ...
                if (isOpen("Fused " + file1 + " & " + file2)) {
                    ...
                    if (isOpen("C1-Fused " + file1 + " & " + file2) && isOpen("C2-Fused " + file1 + " & " + file2)) {
                        ...
                        if (isOpen("Result of C1-Fused " + file1 + " & " + file2)) {
                            ...
                            // Add cropped image width to summary
                            Table.create("Temp");
                            Table.set("Image", 0, fullFileName);
                            Table.set("Cropped_Width", 0, new_width);
                            Table.update;
                            
                            // Append to existing summary or create new if not exists
                            if (isOpen("Summary")) {
                                Table.append("Temp", "Summary");
                            } else {
                                Table.rename("Temp", "Summary");
                            }
