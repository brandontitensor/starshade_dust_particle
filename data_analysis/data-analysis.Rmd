...
comparison_mean_dist <- average_edge_counts %>% 
  ...
  mutate(Observed_edge_counts = Average_Cumulative_Count.x,
  ...
  group_by(Diameter) %>%
  summarize(Observed_edge_counts = round(mean(Observed_edge_counts, na.rm = TRUE)),Modeled_edge_counts = round(mean(Modeled_edge_counts, na.rm = TRUE)),Observed_Calibration_counts = round(mean(Observed_Calibration_counts, na.rm = TRUE)), .groups = "drop") %>% 
  mutate(Modeled_edge_with_adhesion_counts = round(Modeled_edge_counts/Diameter),
...
ggplot() +
 ...
 geom_line(data = comparison_mean_dist, 
  ...
  labs(x = "Diameter (log(microns)^2)", 
       ...
       subtitle = "Red: Observed Edge Counts, Blue: Modeled Edge Counts, Green: Observed Calibration Counts, Blue-dashed: Modeled Edge Counts with Adhesion Factor, Green-dashed: Observed Calibration Counts with Adhesion Factor") +
  theme_minimal() +
  theme(legend.position = "bottom", 
        legend.title = element_text(size = 10),
        legend.text = element_text(size = 8))
...
comparison_all_models <- average_edge_counts %>% 
  ...
  group_by(Diameter) %>%
  summarize(Observed_by_Modeled = mean(Observed_by_Modeled, na.rm = TRUE),Observed_by_Calibration = mean(Observed_by_Calibration, na.rm = TRUE),Modeled_by_Calibration = mean(Modeled_by_Calibration, na.rm = TRUE), Ratio = mean(Ratio, na.rm = TRUE), .groups = "drop") 
...
ggplot() +
 ...
 geom_line(data = comparison_all_models, 
  ...
  labs(x = "Diameter (log(microns)^2)", 
       ...
       subtitle = "Red: Observed by Modeled, Blue: Observed by Calibration, Green: 1 over Diameter") +
  theme_minimal() +
  theme(legend.position = "bottom", 
        legend.title = element_text(size = 10),
        legend.text = element_text(size = 8))
