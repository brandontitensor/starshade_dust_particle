...
load_sample_data <- function(sample_number) {
  ...
  base_dir <- "/Volumes/BRANDONMEGA/Research/Dust_Contamination/Trials/Data/Edges/Edge Measurements"
...
load_calibration_data <- function(sample_number) {
  ...
  summary_path <- sprintf("/Volumes/BRANDONMEGA/Research/Dust_Contamination/Trials/Data/Calibration/Edge Measurements/Bef_%s minus Aft_%s/Summary Bef_%s minus Aft_%s.csv", sample_str, sample_str, sample_str, sample_str)
...
new_edge_factors<- edge_summary_data %>%
  ...
  summarise(
    Total_Width = 0.01 / (sum(width)* 1e-6),  # Convert microns to meters
    .groups = "drop"
  )
...
new_SSLSM_edge_factors<- surface_before_data$summary_data %>%
  group_by(Trial) %>%
  summarise(
    Total_Width = 0.1 / (n() * 600 * 1e-6),  # Convert microns to meters
    .groups = "drop"
  )
...
# Create a comprehensive comparison grid showing all trials and analysis types for scatter data with normalized values
# This normalization makes it easier to compare scatter patterns across different trials
library(tidyverse)
library(gridExtra)
library(grid)
...
for (i in 1:length(trial_order)) {
  ...
  tryCatch({
    ...
    # Base plot for surface cumulative scatter
    p_surf_cum <- ggplot()
  ...
  tryCatch({
    ...
    if(exists(paste0("wit_surface_", trial_num, "_binned"))) {
      ...
      if (!is.null(wit_bin_area_col) && !is.null(wit_bin_scatter_col)) {
        witness_bin_norm <- normalize_scatter_data(witness_bin_data, wit_bin_scatter_col)
        p_surf_bin <- p_surf_bin + 
          geom_line(data = witness_bin_norm,
                    aes_string(x = wit_bin_area_col, y = "normalized_scatter"),
                    color = "#009E73", 
                    linetype = "dashed")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Surface Binned Scatter", "Witness", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Surface Binned Scatter", "Witness", "Dataset does not exist")
    }
    ...
    if(trial_num %in% c(12, 13, 14)) {
      if(exists(paste0("cw_surface_", trial_num, "_binned"))) {
        ...
        if (!is.null(cw_bin_area_col) && !is.null(cw_bin_scatter_col)) {
          cw_bin_norm <- normalize_scatter_data(cw_bin_data, cw_bin_scatter_col)
          p_surf_bin <- p_surf_bin +
            geom_line(data = cw_bin_norm,
                      aes_string(x = cw_bin_area_col, y = "normalized_scatter"),
                      color = "#CC79A7", 
                      linetype = "dotted")
          lines_added <- lines_added + 1
        } else {
          log_missing_data(trial_num, "Surface Binned Scatter", "Calibration", "Missing required columns")
        }
      } else {
        log_missing_data(trial_num, "Surface Binned Scatter", "Calibration", "Dataset does not exist")
      }
    }
  ...
  tryCatch({
    ...
    if(exists(paste0("edge_", trial_num))) {
      ...
      if (!is.null(edge_area_col) && !is.null(edge_scatter_col)) {
        edge_norm <- normalize_scatter_data(edge_data, edge_scatter_col)
        p_edge_cum <- p_edge_cum + 
          geom_line(data = edge_norm,
                    aes_string(x = edge_area_col, y = "normalized_scatter"),
                    color = "#CC79A7", 
                    linetype = "solid")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Cumulative Scatter", "Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Cumulative Scatter", "Edge", "Dataset does not exist")
    }
    ...
    if(exists(paste0("cw_edge_", trial_num))) {
      ...
      if (!is.null(cw_edge_area_col) && !is.null(cw_edge_scatter_col)) {
        cw_edge_norm <- normalize_scatter_data(cw_edge_data, cw_edge_scatter_col)
        p_edge_cum <- p_edge_cum + 
          geom_line(data = cw_edge_norm,
                    aes_string(x = cw_edge_area_col, y = "normalized_scatter"),
                    color = "#0072B2", 
                    linetype = "dashed")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Cumulative Scatter", "CW_Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Cumulative Scatter", "CW_Edge", "Dataset does not exist")
    }
    ...
    if(exists(paste0("SSLSM_edge_", trial_num))) {
      ...
      if (!is.null(SSLSM_edge_area_col) && !is.null(SSLSM_edge_scatter_col)) {
        SSLSM_edge_norm <- normalize_scatter_data(SSLSM_edge_data, SSLSM_edge_scatter_col)
        p_edge_cum <- p_edge_cum + 
          geom_line(data = SSLSM_edge_norm,
                    aes_string(x = SSLSM_edge_area_col, y = "normalized_scatter"),
                    color = "#D55E00", 
                    linetype = "dotdash")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Cumulative Scatter", "SSLSM_Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Cumulative Scatter", "SSLSM_Edge", "Dataset does not exist")
    }
    ...
    if(exists(paste0("WSLSM_edge_", trial_num))) {
      ...
      if (!is.null(WSLSM_edge_area_col) && !is.null(WSLSM_edge_scatter_col)) {
        WSLSM_edge_norm <- normalize_scatter_data(WSLSM_edge_data, WSLSM_edge_scatter_col)
        p_edge_cum <- p_edge_cum + 
          geom_line(data = WSLSM_edge_norm,
                    aes_string(x = WSLSM_edge_area_col, y = "normalized_scatter"),
                    color = "#56B4E9", 
                    linetype = "longdash")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Cumulative Scatter", "WSLSM_Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Cumulative Scatter", "WSLSM_Edge", "Dataset does not exist")
    }
    ...
    if(trial_num %in% c(12, 13, 14)) {
      if(exists(paste0("CSLSM_edge_", trial_num))) {
        ...
        if (!is.null(CSLSM_edge_area_col) && !is.null(CSLSM_edge_scatter_col)) {
          CSLSM_edge_norm <- normalize_scatter_data(CSLSM_edge_data, CSLSM_edge_scatter_col)
          p_edge_cum <- p_edge_cum +
            geom_line(data = CSLSM_edge_norm,
                      aes_string(x = CSLSM_edge_area_col, y = "normalized_scatter"),
                      color = "#F0E442", 
                      linetype = "twodash")
          lines_added <- lines_added + 1
        } else {
          log_missing_data(trial_num, "Edge Cumulative Scatter", "CSLSM_Edge", "Missing required columns")
        }
      } else {
        log_missing_data(trial_num, "Edge Cumulative Scatter", "CSLSM_Edge", "Dataset does not exist")
      }
    }
  ...
  tryCatch({
    ...
    if(exists(paste0("edge_", trial_num, "_binned"))) {
      ...
      if (!is.null(edge_bin_area_col) && !is.null(edge_bin_scatter_col)) {
        edge_bin_norm <- normalize_scatter_data(edge_bin_data, edge_bin_scatter_col)
        p_edge_bin <- p_edge_bin + 
          geom_line(data = edge_bin_norm,
                    aes_string(x = edge_bin_area_col, y = "normalized_scatter"),
                    color = "#CC79A7", 
                    linetype = "solid")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Binned Scatter", "Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Binned Scatter", "Edge", "Dataset does not exist")
    }
    ...
    if(exists(paste0("cw_edge_", trial_num, "_binned"))) {
      ...
      if (!is.null(cw_edge_bin_area_col) && !is.null(cw_edge_bin_scatter_col)) {
        cw_edge_bin_norm <- normalize_scatter_data(cw_edge_bin_data, cw_edge_bin_scatter_col)
        p_edge_bin <- p_edge_bin + 
          geom_line(data = cw_edge_bin_norm,
                    aes_string(x = cw_edge_bin_area_col, y = "normalized_scatter"),
                    color = "#0072B2", 
                    linetype = "dashed")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Binned Scatter", "CW_Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Binned Scatter", "CW_Edge", "Dataset does not exist")
    }
    ...
    if(exists(paste0("SSLSM_edge_", trial_num, "_binned"))) {
      ...
      if (!is.null(SSLSM_edge_bin_area_col) && !is.null(SSLSM_edge_bin_scatter_col)) {
        SSLSM_edge_bin_norm <- normalize_scatter_data(SSLSM_edge_bin_data, SSLSM_edge_bin_scatter_col)
        p_edge_bin <- p_edge_bin + 
          geom_line(data = SSLSM_edge_bin_norm,
                    aes_string(x = SSLSM_edge_bin_area_col, y = "normalized_scatter"),
                    color = "#D55E00", 
                    linetype = "dotdash")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Binned Scatter", "SSLSM_Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Binned Scatter", "SSLSM_Edge", "Dataset does not exist")
    }
    ...
    if(exists(paste0("WSLSM_edge_", trial_num, "_binned"))) {
      ...
      if (!is.null(WSLSM_edge_bin_area_col) && !is.null(WSLSM_edge_bin_scatter_col)) {
        WSLSM_edge_bin_norm <- normalize_scatter_data(WSLSM_edge_bin_data, WSLSM_edge_bin_scatter_col)
        p_edge_bin <- p_edge_bin + 
          geom_line(data = WSLSM_edge_bin_norm,
                    aes_string(x = WSLSM_edge_bin_area_col, y = "normalized_scatter"),
                    color = "#56B4E9", 
                    linetype = "longdash")
        lines_added <- lines_added + 1
      } else {
        log_missing_data(trial_num, "Edge Binned Scatter", "WSLSM_Edge", "Missing required columns")
      }
    } else {
      log_missing_data(trial_num, "Edge Binned Scatter", "WSLSM_Edge", "Dataset does not exist")
    }
    ...
    if(trial_num %in% c(12, 13, 14)) {
      if(exists(paste0("CSLSM_edge_", trial_num, "_binned"))) {
        ...
        if (!is.null(CSLSM_edge_bin_area_col) && !is.null(CSLSM_edge_bin_scatter_col)) {
          CSLSM_edge_bin_norm <- normalize_scatter_data(CSLSM_edge_bin_data, CSLSM_edge_bin_scatter_col)
          p_edge_bin <- p_edge_bin +
            geom_line(data = CSLSM_edge_bin_norm,
                      aes_string(x = CSLSM_edge_bin_area_col, y = "normalized_scatter"),
                      color = "#F0E442", 
                      linetype = "twodash")
          lines_added <- lines_added + 1
        } else {
          log_missing_data(trial_num, "Edge Binned Scatter", "CSLSM_Edge", "Missing required columns")
        }
      } else {
        log_missing_data(trial_num, "Edge Binned Scatter", "CSLSM_Edge", "Dataset does not exist")
      }
    }
...
tryCatch({
  ...
  surface_avg_scatter_cum_norm_plot <- ggplot()
  surface_avg_air_scatter_cum_norm_plot <- ggplot()
  ...
  if(exists("surface_average")) {
    ...
    if (!is.null(s_avg_area_col) && !is.null(s_avg_scatter_col)) {
      surface_avg_norm <- normalize_scatter_data(surface_average, s_avg_scatter_col)
      surface_avg_scatter_cum_norm_plot <- surface_avg_scatter_cum_norm_plot +
        geom_line(data = surface_avg_norm,
                  aes_string(x = s_avg_area_col, y = "normalized_scatter"),
                  color = "#D55E00", 
                  linetype = "solid")
      lines_added_avg <- lines_added_avg + 1
    } else {
      log_missing_data("Average", "Surface Cumulative Scatter", "Surface", "Missing required columns")
    }
  } else {
    log_missing_data("Average", "Surface Cumulative Scatter", "Surface", "Dataset does not exist")
  }
  ...
  if(exists("wit_surface_average")) {
    ...
    if (!is.null(wit_avg_area_col) && !is.null(wit_avg_scatter_col)) {
      wit_avg_norm <- normalize_scatter_data(wit_surface_average, wit_avg_scatter_col)
      surface_avg_scatter_cum_norm_plot <- surface_avg_scatter_cum_norm_plot +
        geom_line(data = wit_avg_norm,
                  aes_string(x = wit_avg_area_col, y = "normalized_scatter"),
                  color = "#009E73", 
                  linetype = "dashed")
      lines_added_avg <- lines_added_avg + 1
    } else {
      log_missing_data("Average", "Surface Cumulative Scatter", "Witness", "Missing required columns")
    }
  } else {
    log_missing_data("Average", "Surface Cumulative Scatter", "Witness", "Dataset does not exist")
  }
  ...
  if(exists("cw_surface_average")) {
    ...
    if (!is.null(cw_avg_area_col) && !is.null(cw_avg_scatter_col)) {
      cw_avg_norm <- normalize_scatter_data(cw_surface_average, cw_avg_scatter_col)
      surface_avg_scatter_cum_norm_plot <- surface_avg_scatter_cum_norm_plot +
        geom_line(data = cw_avg_norm,
                  aes_string(x = cw_avg_area_col, y = "normalized_scatter"),
                  color = "#CC79A7", 
                  linetype = "dotted")
      lines_added_avg <- lines_added_avg + 1
    } else {
      log_missing_data("Average", "Surface Cumulative Scatter", "Calibration", "Missing required columns")
    }
  } else {
    log_missing_data("Average", "Surface Cumulative Scatter", "Calibration", "Dataset does not exist")
  }
  ...
  if(exists("surface_average_air")) {
    ...
    if (!is.null(s_avg_air_area_col) && !is.null(s_avg_air_scatter_col)) {
      surface_avg_air_norm <- normalize_scatter_data(surface_average_air, s_avg_air_scatter_col)
      surface_avg_air_scatter_cum_norm_plot <- surface_avg_air_scatter_cum_norm_plot +
        geom_line(data = surface_avg_air_norm,
                  aes_string(x = s_avg_air_area_col, y = "normalized_scatter"),
                  color = "#D55E00", 
                  linetype = "solid")
      lines_added_air <- lines_added_air + 1
    } else {
      log_missing_data("Air Average", "Surface Cumulative Scatter", "Surface", "Missing required columns")
    }
  } else {
    log_missing_data("Air Average", "Surface Cumulative Scatter", "Surface", "Dataset does not exist")
  }
  ...
  if(exists("wit_surface_average_air")) {
    ...
    if (!is.null(wit_avg_air_area_col) && !is.null(wit_avg_air_scatter_col)) {
      wit_avg_air_norm <- normalize_scatter_data(wit_surface_average_air, wit_avg_air_scatter_col)
      surface_avg_air_scatter_cum_norm_plot <- surface_avg_air_scatter_cum_norm_plot +
        geom_line(data = wit_avg_air_norm,
                  aes_string(x = wit_avg_air_area_col, y = "normalized_scatter"),
                  color = "#009E73", 
                  linetype = "dashed")
      lines_added_air <- lines_added_air + 1
    } else {
      log_missing_data("Air Average", "Surface Cumulative Scatter", "Witness", "Missing required columns")
    }
  } else {
    log_missing_data("Air Average", "Surface Cumulative Scatter", "Witness", "Dataset does not exist")
  }
  ...
  if(exists("cw_surface_average_air")) {
    ...
    if (!is.null(cw_avg_air_area_col) && !is.null(cw_avg_air_scatter_col)) {
      cw_avg_air_norm <- normalize_scatter_data(cw_surface_average_air, cw_avg_air_scatter_col)
      surface_avg_air_scatter_cum_norm_plot <- surface_avg_air_scatter_cum_norm_plot +
        geom_line(data = cw_avg_air_norm,
                  aes_string(x = cw_avg_air_area_col, y = "normalized_scatter"),
                  color = "#CC79A7", 
                  linetype = "dotted")
      lines_added_air <- lines_added_air + 1
    } else {
      log_missing_data("Air Average", "Surface Cumulative Scatter", "Calibration", "Missing required columns")
    }
  } else {
    log_missing_data("Air Average", "Surface Cumulative Scatter", "Calibration", "Dataset does not exist")
  }
  
  # Finalize plots or use fallback
  if(lines_added_avg == 0) {
    surface_scatter_cumulative_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
    log_missing_data("Average", "Surface Cumulative Scatter", "ALL", "No data lines available - using fallback")
  } else {
    surface_avg_scatter_cum_norm_plot <- surface_avg_scatter_cum_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    surface_scatter_cumulative_norm_plots[[length(trial_order) + 1]] <- surface_avg_scatter_cum_norm_plot
  }
  
  if(lines_added_air == 0) {
    surface_scatter_cumulative_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
    log_missing_data("Air Average", "Surface Cumulative Scatter", "ALL", "No data lines available - using fallback")
  } else {
    surface_avg_air_scatter_cum_norm_plot <- surface_avg_air_scatter_cum_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    surface_scatter_cumulative_norm_plots[[length(trial_order) + 2]] <- surface_avg_air_scatter_cum_norm_plot
  }
}, error = function(e) {
  surface_scatter_cumulative_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
  surface_scatter_cumulative_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
  log_missing_data("Average", "Surface Cumulative Scatter", "ERROR", paste("Error occurred:", e$message))
})

# Surface binned scatter average with normalization and logging
tryCatch({
  lines_added_avg <- 0
  lines_added_air <- 0
  
  surface_avg_scatter_bin_norm_plot <- ggplot()
  surface_avg_air_scatter_bin_norm_plot <- ggplot()
  ...
  if(exists("surface_average_binned")) {
    ...
    if (!is.null(s_avg_bin_area_col) && !is.null(s_avg_bin_scatter_col)) {
      surface_avg_bin_norm <- normalize_scatter_data(surface_average_binned, s_avg_bin_scatter_col)
      surface_avg_scatter_bin_norm_plot <- surface_avg_scatter_bin_norm_plot +
        geom_line(data = surface_avg_bin_norm,
                  aes_string(x = s_avg_bin_area_col, y = "normalized_scatter"),
                  color = "#D55E00", 
                  linetype = "solid")
      lines_added_avg <- lines_added_avg + 1
    } else {
      log_missing_data("Average", "Surface Binned Scatter", "Surface", "Missing required columns")
    }
  } else {
    log_missing_data("Average", "Surface Binned Scatter", "Surface", "Dataset does not exist")
  }
  ...
  if(exists("wit_surface_average_binned")) {
    ...
    if (!is.null(wit_avg_bin_area_col) && !is.null(wit_avg_bin_scatter_col)) {
      wit_avg_bin_norm <- normalize_scatter_data(wit_surface_average_binned, wit_avg_bin_scatter_col)
      surface_avg_scatter_bin_norm_plot <- surface_avg_scatter_bin_norm_plot +
        geom_line(data = wit_avg_bin_norm,
                  aes_string(x = wit_avg_bin_area_col, y = "normalized_scatter"),
                  color = "#009E73", 
                  linetype = "dashed")
      lines_added_avg <- lines_added_avg + 1
    } else {
      log_missing_data("Average", "Surface Binned Scatter", "Witness", "Missing required columns")
    }
  } else {
    log_missing_data("Average", "Surface Binned Scatter", "Witness", "Dataset does not exist")
  }
  ...
  if(exists("cw_surface_average_binned")) {
    ...
    if (!is.null(cw_avg_bin_area_col) && !is.null(cw_avg_bin_scatter_col)) {
      cw_avg_bin_norm <- normalize_scatter_data(cw_surface_average_binned, cw_avg_bin_scatter_col)
      surface_avg_scatter_bin_norm_plot <- surface_avg_scatter_bin_norm_plot +
        geom_line(data = cw_avg_bin_norm,
                  aes_string(x = cw_avg_bin_area_col, y = "normalized_scatter"),
                  color = "#CC79A7", 
                  linetype = "dotted")
      lines_added_avg <- lines_added_avg + 1
    } else {
      log_missing_data("Average", "Surface Binned Scatter", "Calibration", "Missing required columns")
    }
  } else {
    log_missing_data("Average", "Surface Binned Scatter", "Calibration", "Dataset does not exist")
  }
  ...
  if(exists("surface_average_air_binned")) {
    ...
    if (!is.null(s_avg_air_bin_area_col) && !is.null(s_avg_air_bin_scatter_col)) {
      surface_avg_air_bin_norm <- normalize_scatter_data(surface_average_air_binned, s_avg_air_bin_scatter_col)
      surface_avg_air_scatter_bin_norm_plot <- surface_avg_air_scatter_bin_norm_plot +
        geom_line(data = surface_avg_air_bin_norm,
                  aes_string(x = s_avg_air_bin_area_col, y = "normalized_scatter"),
                  color = "#D55E00", 
                  linetype = "solid")
      lines_added_air <- lines_added_air + 1
    } else {
      log_missing_data("Air Average", "Surface Binned Scatter", "Surface", "Missing required columns")
    }
  } else {
    log_missing_data("Air Average", "Surface Binned Scatter", "Surface", "Dataset does not exist")
  }
  ...
  if(exists("wit_surface_average_air_binned")) {
    ...
    if (!is.null(wit_avg_air_bin_area_col) && !is.null(wit_avg_air_bin_scatter_col)) {
      wit_avg_air_bin_norm <- normalize_scatter_data(wit_surface_average_air_binned, wit_avg_air_bin_scatter_col)
      surface_avg_air_scatter_bin_norm_plot <- surface_avg_air_scatter_bin_norm_plot +
        geom_line(data = wit_avg_air_bin_norm,
                  aes_string(x = wit_avg_air_bin_area_col, y = "normalized_scatter"),
                  color = "#009E73", 
                  linetype = "dashed")
      lines_added_air <- lines_added_air + 1
    } else {
      log_missing_data("Air Average", "Surface Binned Scatter", "Witness", "Missing required columns")
    }
  } else {
    log_missing_data("Air Average", "Surface Binned Scatter", "Witness", "Dataset does not exist")
  }
  ...
  if(exists("cw_surface_average_air_binned")) {
    ...
    if (!is.null(cw_avg_air_bin_area_col) && !is.null(cw_avg_air_bin_scatter_col)) {
      cw_avg_air_bin_norm <- normalize_scatter_data(cw_surface_average_air_binned, cw_avg_air_bin_scatter_col)
      surface_avg_air_scatter_bin_norm_plot <- surface_avg_air_scatter_bin_norm_plot +
        geom_line(data = cw_avg_air_bin_norm,
                  aes_string(x = cw_avg_air_bin_area_col, y = "normalized_scatter"),
                  color = "#CC79A7", 
                  linetype = "dotted")
      lines_added_air <- lines_added_air + 1
    } else {
      log_missing_data("Air Average", "Surface Binned Scatter", "Calibration", "Missing required columns")
    }
  } else {
    log_missing_data("Air Average", "Surface Binned Scatter", "Calibration", "Dataset does not exist")
  }
  
  # Finalize plots or use fallback
  if(lines_added_avg == 0) {
    surface_scatter_binned_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
    log_missing_data("Average", "Surface Binned Scatter", "ALL", "No data lines available - using fallback")
  } else {
    surface_avg_scatter_bin_norm_plot <- surface_avg_scatter_bin_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    surface_scatter_binned_norm_plots[[length(trial_order) + 1]] <- surface_avg_scatter_bin_norm_plot
  }
  
  if(lines_added_air == 0) {
    surface_scatter_binned_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
    log_missing_data("Air Average", "Surface Binned Scatter", "ALL", "No data lines available - using fallback")
  } else {
    surface_avg_air_scatter_bin_norm_plot <- surface_avg_air_scatter_bin_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    surface_scatter_binned_norm_plots[[length(trial_order) + 2]] <- surface_avg_air_scatter_bin_norm_plot
  }
}, error = function(e) {
  surface_scatter_binned_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
  surface_scatter_binned_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
  log_missing_data("Average", "Surface Binned Scatter", "ERROR", paste("Error occurred:", e$message))
})

# Edge cumulative scatter average with normalization and logging
tryCatch({
  lines_added_avg <- 0
  lines_added_air <- 0
  
  edge_avg_scatter_cum_norm_plot <- ggplot()
  edge_avg_air_scatter_cum_norm_plot <- ggplot()
  
  # Try standard average data for edge cumulative scatter
  edge_datasets <- list(
    list(name = "edge_average", color = "#CC79A7", linetype = "solid", label = "Edge"),
    list(name = "cw_edge_average", color = "#0072B2", linetype = "dashed", label = "CW_Edge"),
    list(name = "SSLSM_edge_average", color = "#D55E00", linetype = "dotdash", label = "SSLSM_Edge"),
  ...
  for(dataset in edge_datasets) {
    if(exists(dataset$name)) {
      data <- get(dataset$name)
      area_col <- get_column_name(data, c("log_area", "Area"))
      scatter_col <- get_column_name(data, c("Average_Scatter"))
      
      if (!is.null(area_col) && !is.null(scatter_col)) {
        data_norm <- normalize_scatter_data(data, scatter_col)
        edge_avg_scatter_cum_norm_plot <- edge_avg_scatter_cum_norm_plot +
          geom_line(data = data_norm,
                    aes_string(x = area_col, y = "normalized_scatter"),
                    color = dataset$color, 
                    linetype = dataset$linetype)
        ...
        log_missing_data("Average", "Edge Cumulative Scatter", dataset$label, "Missing required columns")
      }
    } else {
      log_missing_data("Average", "Edge Cumulative Scatter", dataset$label, "Dataset does not exist")
    }
  }
  ...
  for(dataset in edge_air_datasets) {
    if(exists(dataset$name)) {
      ...
      if (!is.null(area_col) && !is.null(scatter_col)) {
        data_norm <- normalize_scatter_data(data, scatter_col)
        edge_avg_air_scatter_cum_norm_plot <- edge_avg_air_scatter_cum_norm_plot +
          geom_line(data = data_norm,
                    aes_string(x = area_col, y = "normalized_scatter"),
                    color = dataset$color, 
                    linetype = dataset$linetype)
        ...
        log_missing_data("Air Average", "Edge Cumulative Scatter", dataset$label, "Missing required columns")
      }
    } else {
      log_missing_data("Air Average", "Edge Cumulative Scatter", dataset$label, "Dataset does not exist")
    }
  }
  
  # Finalize plots or use fallback
  if(lines_added_avg == 0) {
    edge_scatter_cumulative_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
    log_missing_data("Average", "Edge Cumulative Scatter", "ALL", "No data lines available - using fallback")
  } else {
    edge_avg_scatter_cum_norm_plot <- edge_avg_scatter_cum_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    edge_scatter_cumulative_norm_plots[[length(trial_order) + 1]] <- edge_avg_scatter_cum_norm_plot
  }
  
  if(lines_added_air == 0) {
    edge_scatter_cumulative_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
    log_missing_data("Air Average", "Edge Cumulative Scatter", "ALL", "No data lines available - using fallback")
  } else {
    edge_avg_air_scatter_cum_norm_plot <- edge_avg_air_scatter_cum_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    edge_scatter_cumulative_norm_plots[[length(trial_order) + 2]] <- edge_avg_air_scatter_cum_norm_plot
  }
}, error = function(e) {
  edge_scatter_cumulative_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
  edge_scatter_cumulative_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
  log_missing_data("Average", "Edge Cumulative Scatter", "ERROR", paste("Error occurred:", e$message))
})

# Edge binned scatter average with normalization and logging
tryCatch({
  lines_added_avg <- 0
  lines_added_air <- 0
  
  edge_avg_scatter_bin_norm_plot <- ggplot()
  edge_avg_air_scatter_bin_norm_plot <- ggplot()
  
  # Define datasets for edge binned scatter average
  edge_bin_datasets <- list(
    list(name = "edge_average_binned", color = "#CC79A7", linetype = "solid", label = "Edge"),
    list(name = "cw_edge_average_binned", color = "#0072B2", linetype = "dashed", label = "CW_Edge"),
    list(name = "SSLSM_edge_average_binned", color = "#D55E00", linetype = "dotdash", label = "SSLSM_Edge"),
  ...
  for(dataset in edge_bin_datasets) {
    if(exists(dataset$name)) {
      ...
      if (!is.null(area_col) && !is.null(scatter_col)) {
        data_norm <- normalize_scatter_data(data, scatter_col)
        edge_avg_scatter_bin_norm_plot <- edge_avg_scatter_bin_norm_plot +
          geom_line(data = data_norm,
                    aes_string(x = area_col, y = "normalized_scatter"),
                    color = dataset$color, 
                    linetype = dataset$linetype)
        ...
        log_missing_data("Average", "Edge Binned Scatter", dataset$label, "Missing required columns")
      }
    } else {
      log_missing_data("Average", "Edge Binned Scatter", dataset$label, "Dataset does not exist")
    }
  }
  ...
  for(dataset in edge_air_bin_datasets) {
    if(exists(dataset$name)) {
      ...
      if (!is.null(area_col) && !is.null(scatter_col)) {
        data_norm <- normalize_scatter_data(data, scatter_col)
        edge_avg_air_scatter_bin_norm_plot <- edge_avg_air_scatter_bin_norm_plot +
          geom_line(data = data_norm,
                    aes_string(x = area_col, y = "normalized_scatter"),
                    color = dataset$color, 
                    linetype = dataset$linetype)
        ...
        log_missing_data("Air Average", "Edge Binned Scatter", dataset$label, "Missing required columns")
      }
    } else {
      log_missing_data("Air Average", "Edge Binned Scatter", dataset$label, "Dataset does not exist")
    }
  }
  
  # Finalize plots or use fallback
  if(lines_added_avg == 0) {
    edge_scatter_binned_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
    log_missing_data("Average", "Edge Binned Scatter", "ALL", "No data lines available - using fallback")
  } else {
    edge_avg_scatter_bin_norm_plot <- edge_avg_scatter_bin_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    edge_scatter_binned_norm_plots[[length(trial_order) + 1]] <- edge_avg_scatter_bin_norm_plot
  }
  
  if(lines_added_air == 0) {
    edge_scatter_binned_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
    log_missing_data("Air Average", "Edge Binned Scatter", "ALL", "No data lines available - using fallback")
  } else {
    edge_avg_air_scatter_bin_norm_plot <- edge_avg_air_scatter_bin_norm_plot +
      labs(
        x = expression(log[10](Area)),
      ...
      compact_theme
    edge_scatter_binned_norm_plots[[length(trial_order) + 2]] <- edge_avg_air_scatter_bin_norm_plot
  }
}, error = function(e) {
  edge_scatter_binned_norm_plots[[length(trial_order) + 1]] <- create_fallback_plot()
  edge_scatter_binned_norm_plots[[length(trial_order) + 2]] <- create_fallback_plot()
  log_missing_data("Average", "Edge Binned Scatter", "ERROR", paste("Error occurred:", e$message))
})
