...
# Filter surface particles by minimum area (1 )
cw_surface_before_particles <- Cwsurface_before_data$particle_data %>%
  filter(Area > 1)
cw_surface_after_particles <- Cwsurface_after_data$particle_data %>%
  filter(Area > 1)
cw_combined_surface_data <- bind_rows(cw_surface_before_particles,cw_surface_after_particles)
...
# Calculate area-based counts for before and after data
# Normalizes each sample to 0.1 m² for standardized comparison
cw_surface_before_counts <- cw_surface_before_particles %>%
  group_by(Trial, Sample) %>%
  group_modify(~ {
...
```{r Multiple_line_analysis, include=FALSE, echo = FALSE, eval=TRUE}
# This section simulates drawing a line across surfaces and analyzes how particles intersect with it
