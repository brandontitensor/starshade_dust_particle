...
edge_model_data <- surface_count_diff %>%
  left_join(surface_before_data$trial_areas, by = "Trial") %>%
  mutate(
    Normalized_Area = (Positive_Diff * Area) / (.1 * 10^12),
...
comparison_mean_dist <- average_edge_counts %>% 
  full_join(edge_model_data[,c(1,3,15)], by = c("Trial", "Area")) %>% 
